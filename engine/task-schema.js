/**
 * Task Schema — JS shim that reads constants from the canonical JSON
 * (../task-schema.json at the repo root) and adds validation logic.
 *
 * To change rules, edit task-schema.json. This file just consumes them.
 */
...
// Re-exported constants — every other module (and the UI, via GET /schema)
// gets these from the same place.
...
// Expose the raw schema document too — for any consumer that wants the full
// JSON (e.g., the Lambda's GET /schema route, or a future generator script).
 ...
 * @param {object} [opts]
 * @param {string[]} [opts.knownProjects]  If provided, repo_hints are checked
 *                                         against this list. Pass an empty array
 *                                         to skip the cross-check.
 * @param {boolean} [opts.requireId=true]  Set false when validating a task
 *                                         that hasn't been assigned an id yet
 *                                         (API creation flow).
